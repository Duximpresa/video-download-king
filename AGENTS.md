...
- 视频编码面板依次为“图像”“比特率调整”“音频设置”“硬件加速”“文件与输出选项”。
- H.264 的 Shutter 风格自动码率为 `宽 × 高 × FPS × 8 × 2 ÷ 165888`；“好的”为两倍，“最好”为四倍。
- VBR、CBR 使用目标码率；CQ 使用 1–51 的质量值。最大限度会产生 `maxrate` 和两倍 `bufsize`。
- 二次编码仅用于 CPU 的 VBR/CBR；第一遍不得编码音频，pass 日志在结束、失败和取消后都要清理。
- 固定分辨率默认等比缩放并补黑边，不能静默拉伸。比例、自适应边长和竖构图必须输出偶数尺寸。
- 音频转换限定为 MP4 兼容子集；复制的源音频不兼容 MP4 时回退 AAC 并记录警告。
- 硬件解码、滤镜和编码分别探测；不兼容的硬件滤镜回退软件，硬件编码失败仍询问是否使用 CPU。
...
- 格式预设、输出模式和命名。
- 隐藏窗口子进程。
- 图像滤镜、Shutter 风格码率、目标大小、二次编码和硬件参数。
