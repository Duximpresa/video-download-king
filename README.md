...
```text
release/VideoDownloadKing-v0.2.0-Windows-x64.zip
```

## 使用说明

1. 输入 YouTube 单视频链接并点击“分析链接”。
2. 选择“视频+音频”“仅视频”“仅音频”或“高级流组合”。
3. 可使用 `{title}`、`{id}`、`{channel}`、`{platform}`、`{upload_date}`、`{download_date}` 自定义文件名。
4. 按需勾选下载封面、下载字幕或将封面嵌入视频。默认字幕语言为 `zh-Hans,zh.*,en.*`。
5. 兼容 MP4 区域可选择 CPU 或 GPU。GPU 厂商必须通过启动时实际编码检测才可选择。
6. 自动码率优先沿用源视频实际码率；转码文件默认追加 `_H264_AAC`，也可自定义或关闭后缀。
